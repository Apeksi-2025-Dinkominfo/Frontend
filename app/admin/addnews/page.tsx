        ...
        <tbody>
          {stories.map((story, index) => (
            <tr key={story.id} className="border-b border-gray-200">
              ...
              <td className="py-3 px-4">{formatDate(story.date)}</td>{' '}
...
const StoryManagementPage: React.FC = () => {
  ...
  const handleEdit = (story: Story) => {
    ...
    setNewStory({
      ...story,
      date: new Date(story.date), // Convert string to Date object
      images: null // Reset images to null as we don't have the file object
    })
    setPreviewUrl(story.images as string) // Assuming story.images is a URL string
    setOpenDialog(true)
  }
  ...
  const handleSaveStory = async () => {
    ...
    const formattedDate = new Date(newStory.date).toISOString();
    formData.append('date', formattedDate);

    formData.append('body', newStory.body);

    // Only append the image file if it exists
    if (newStory.images instanceof File) {
  ...
  const handleImageUpload = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      setNewStory({ ...newStory, images: file });
  ...
  return (
    <div className="flex h-screen flex-col md:flex-row">
      <div className="flex-1 p-2 ml-1 overflow-y-auto">
        ...
        {/* Assume StoryTable component exists */}
        <StoryTable
          stories={stories}
        ...
        <div className="fixed bottom-16 right-16">
          ...
          >
            <SpeedDialAction icon={<AddIcon />} tooltipTitle="Add News" />
          </SpeedDial>
        </div>
        ...
        <Dialog open={openDialog} onClose={() => setOpenDialog(false)}>
          <DialogTitle>
            {editStoryId !== null ? 'Edit Story' : 'Add New Story'}
          </DialogTitle>
          <DialogContent>
            <TextField
              autoFocus
              ...
              onChange={(e) =>
                setNewStory({ ...newStory, tittle: e.target.value })
              }
            />
            <TextField
              margin="dense"
            ...
            <TextField
              ...
              value={newStory.body}
              onChange={(e) =>
                setNewStory({ ...newStory, body: e.target.value })
              }
            />
            {previewUrl && (
              <img src={previewUrl} alt="Preview" className="w-full mt-4" />
            )}
            <input
              type="file"
              accept="image/*"
